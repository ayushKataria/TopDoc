...
async function compareHashPassword(req_password, saved_Password){
    try {
        const comparehashedPassword = await bcrypt.compare(req_password , saved_Password)
    ...
    try {
        ...
        if(req.hasOwnProperty("emailId")){
            ...
            if(userData['hits']['total']['value'] > 0){
                throw{ statuscode:499, err: "access denied", message: "This emailId is already registered"}
            }
            else{
                ...
                let updateResult = await Promise.all ([
                    esdb.updateData(userQueryUpdate, "user")
            ...
            if(userData['hits']['total']['value'] > 0){
                throw{ statuscode:499, err: "access denied", message: "This mobile number is already registered"}
            }
            else{
                hashpassword = await hashPassword(req.password)
                id = uuid.v1()
        ...
        if(req.hasOwnProperty("emailId")){
            ...
            if(userData['hits']['total']['value'] > 0){
                ...
                if(isVerified){
                    ...
                    result = { statuscode:200, message: "Authorization successfull", "token": token}
                }else{
                    result = { statuscode:401, message: "Authorization failed"}
                }
            }
            else{
                throw{ statuscode:401, err: "access denied", message: "Authorization failed"}
            }
        }
            ...
            if(userData['hits']['total']['value'] > 0){
                ...
                if(isVerified){
                    ...
                    if(!updateResult[0].hasOwnProperty('status'))
                        result = { statuscode:200, message: "password successfully changed"}
                }else{
...
async function updateProfile(req){
    try {
        ...
        if(!updateResult[0].hasOwnProperty('status'))
            ...
            result = { statuscode:400, message: "update unsuccessfull"}
        return result
    } catch (error) {
        if (error.statuscode){
            throw error
